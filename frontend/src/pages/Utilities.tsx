...
export const Utilities = () => {
  ...
  const formatStory = (story: { value: string; type: string }[]) => {
    const formattedStory = story.map((piece) => 
      piece.type === "blank" ? (
        <span className="text-orange-300 italic underline font-base">{piece.value}</span>
      ) : (
        piece.value
      )
    );
    return formattedStory.reduce((paragraphs: JSX.Element[], piece, index) => {
      ...
      if (typeof piece === 'string' && piece.includes('\n')) {
        ...
        return [
          ...
          <p key={paragraphs.length - 1}>{[...lastParagraph.props.children, current]}</p>,
      ...
      return [
        ...paragraphs.slice(0, -1),
        <p key={paragraphs.length - 1}>{[...lastParagraph.props.children, piece]}</p>
      ];
    }, []);
  };
          ...
          <div className="flex flex-col gap-4 flex-1 overflow-y-auto ">
            ...
            <div className="flex gap-2">
              ...
              <div className="flex flex-col gap-2 flex-1">
                <Card type="dark" padding="tight" className="flex flex-col gap-4 flex-1 font-light text-lg tracking-wide leading-relaxed">
                  {storyRevealed
                    ? formatStory(story)
